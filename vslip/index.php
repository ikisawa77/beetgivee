...
if(!function_exists($database_check[$database_type])&&$database_type!=5){
...
if(!class_exists($database_check[$database_type])&&$database_type==5){
	die ('<h1>PHP ของคุณไม่รองรับ การเชื่อมฐานข้อมูลแบบ '.$database_check[$database_type].' ต้องเปิดส่วนเสริมของ PHP ให้รองรับ หรือเปลี่ยนการเชื่อมต่อแบบอื่น ที่ไฟล์ Config.php ตัวแปล $database_type </h1>');
}
...
if(isset($_POST["ref1"])){
	if($_POST["ref1"]==""){
		...
		if($_POST["qrcode"]){
			...
			header( "location:https://".$_POST["pathner_domain"]."/vslip/scan.php?return=".urlencode($actual_link) );
		}
	}
}
...
if($_GET["qrcode"]&&$_SESSION["ref1"]){
	...
	//-----------------------------------------------------------------------------------------------------------------
		...
		$database_update=array(
			...
			'5'=>'$res = $conn->prepare("update $database_table set $database_point_field = $database_point_field + $point where $database_user_field = \'$ref1\' ");
				$res->execute();',
		);
    ...
    <div class="page-wrapper bg-gra-02 p-t-30 p-b-100 font-poppins">
        <div class="wrapper wrapper--w680">
            <div class="card card-4">
                <div class="card-body"><img src="b1.jpg" width="20%"><img src="scan.png" width="20%">  
                    ...
                    <form method="POST" id="userform" >
						...
						<div class="input-group">
                            ...
                            <div class="rs-select2 js-select-simple select--no-search">
								...
								<div class="card card-4">
									...
									<tr >
										...
										<td > 
											...
											<div class="label"><b>เลขบัญชี : </b><?=substr($account_no,0,3)?>-<?=substr($account_no,3,1)?>-<?=substr($account_no,4,5)?>-<?=substr($account_no,9)?></div>
								...
								<input value="<?=$connect_api["pathner_domain"]?>" type="hidden" name="pathner_domain">
