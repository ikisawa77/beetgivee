...
function MemberResultsPreview() {
  return <main className="results-shell"><header className="results-header"><Link href="/" className="back-link"><ArrowLeft size={16} />กลับหน้าทีเด็ด</Link><div className="simple-brand">Bet<span>Pay</span></div><div className="results-header-note">Member · ดูสรุปผลได้</div></header><div className="results-page"><div className="simple-kicker">MEMBER PREVIEW</div><h1>สรุปผลการแข่งขัน</h1><p className="simple-lead">บัญชี Member ดูภาพรวมผลการแข่งขันได้ อัปเกรดเป็น Silver เพื่อเปิดรายละเอียดทุกคู่และสถิติเต็มรูปแบบ</p><section className="result-overview"><div><span className="overview-label">ผลสำเร็จล่าสุด</span><strong>7 / 9</strong><span className="overview-sub">อัตราเข้า 77.8%</span></div><div className="overview-stat"><span>สถานะสมาชิก</span><b className="green-text">Member</b></div><div className="overview-stat"><span>ปลดล็อกเต็ม</span><b>Silver</b></div><div className="overview-stat"><span>อัปเกรด</span><b className="red-text">พร้อมใช้งาน</b></div></section><section className="results-locked-panel"><div className="results-access-mark"><LockKeyhole size={22} /></div><h2>รายละเอียดผลการแข่งขันอยู่ใน Silver</h2><p>ดูทีมที่ทาย ผลจริง สถานะทายถูก/ผิด และสถิติรายคู่ได้ทันทีเมื่ออัปเกรดสมาชิก</p><Link className="payment-submit" href="/plans"><Sparkles size={16} />อัปเกรดเป็น Silver</Link></section></div></main>;
}
...
export default async function ResultsPage() {
  ...
  const tier = session?.role === "ADMIN" ? "GOLD" : session ? getMembershipTier(session.userId) : "MEMBER";
  if (tierRank(await tier) < 2) return <MemberResultsPreview />;
