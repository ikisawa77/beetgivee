...
export default function HomePage() {
  ...
  return <main className="site-shell">
    ...
    <div className="page" id="tips"><div className="toolbar"><h2>ทีเด็ดฟุตบอลวันนี้</h2><div className="toolbar-meta"><span className="live-dot" />กำลังเปิดรับข้อมูล <span>·</span> {allPicks.length} คู่</div></div>
      <div className="dashboard-grid"><section className="fixture-panel"><div className="panel-head"><strong>โปรแกรมพร้อมทีเด็ด</strong><small>Visitor เห็น 2 คู่ · Silver เห็นทั้งหมด</small></div>{leagues.map((league) => <div className="league-block" key={league.name}><div className="league-title"><div className="league-name"><span className="league-mark">{league.country}</span>{league.name}</div><span className="league-count">{league.picks.length} คู่</span></div><div className="table-wrap"><table className="fixture-table"><thead><tr><th>เวลา</th><th>เจ้าบ้าน</th><th>ทีเด็ด</th><th>ทีมเยือน</th><th>ความมั่นใจ</th><th>ราคา</th></tr></thead><tbody>{league.picks.map((pick) => <tr key={pick.id} className={pick.locked ? "locked-row" : ""} data-testid="published-pick"><td className="fixture-time">{pick.time}</td><td><span className="team-name home">{pick.home}</span></td><td>{pick.locked ? <span className="lock-chip"><LockKeyhole size={12} />สำหรับสมาชิก</span> : <span className="pick-pill"><Trophy size={12} />{pick.pick}</span>}</td><td><span className={pick.locked ? "locked-cell" : "team-name"}>{pick.locked ? "ล็อกข้อมูล" : pick.away}</span></td><td className={pick.locked ? "locked-cell" : "confidence"}>{pick.locked ? "—" : `${pick.confidence}%`}</td><td className={pick.locked ? "locked-cell" : "odds"}>{pick.locked ? "—" : pick.odds}</td></tr>)}</tbody></table></div></div>)}</section>
        <aside className="side-panel"><div className="panel-head"><strong>อันดับความมั่นใจ</strong><small>AI + ทีมงาน</small></div><div className="rank-list">{allPicks.filter((pick) => !pick.locked).sort((a, b) => b.confidence - a.confidence).slice(0, 4).map((pick, index) => <div className="rank-row" key={pick.id}><span className="rank-no">0{index + 1}</span><div><div className="rank-team">{pick.pick}</div><div className="rank-league">{pick.home} vs {pick.away}</div></div><span className="rank-score">{pick.confidence}%</span></div>)}</div><div className="upgrade"><strong>ปลดล็อกทีเด็ดทั้งหมด</strong><p>สมาชิก Silver เห็นทุกคู่ พร้อมบทวิเคราะห์และประวัติผลการแข่งขัน</p><button>ดูแพ็กเกจสมาชิก →</button></div></aside>
